...
// --- Menu prompt ---
Console.Write("Choose agent mode: [C] Coding/Builder, [B] Builder only, [A] Ask/question (default C): ");
var choice = Console.ReadLine()?.Trim().ToUpperInvariant() ?? "C";
if (!choice.StartsWith('C') && !choice.StartsWith('B') && !choice.StartsWith('A'))
{
...
{
    case 'C':
        await RunCodingWorkflowMode(skillsDir);
        break;
    case 'B':
...
// --- Coding/Builder mode: full workflow pipeline (planner → router → builder) ---
static async Task RunCodingWorkflowMode(string skillsDir)
{
